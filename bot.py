...
async def create_ticket_with_details(guild, user, ticket_type, tier, trader, giving, receiving, both_join, tip):
    try:
        ...
        tier_role_id = mm_roles.get(tier)
        ...
        print(f'mm ticket creation failed: {e}')
        raise
...
@bot.command(name='gf', aliases=['gambleflip', 'coinflip'])
async def battle_flip(ctx, user1: discord.Member, vs: str, user2: discord.Member, mode: str, amount: int):
    ...
    if vs.lower() != 'vs':
        return await ctx.reply('❌ Format: `$gf @user1 vs @user2 ft/bo amount`')
    ...
    if amount > 30:
        return await ctx.reply('❌ Maximum amount is 30!')
...
async def help_command(ctx):
    ...
    embed.add_field(
        ...
        value=(
            '```ini\n'
            '[GF] Start coinflip battle\n\n'
            'Usage:\n'
            '$gf @user1 vs @user2 ft 10\n'
            '(First to 10 points)\n\n'
            '$gf @user1 vs @user2 bo 10\n'
            '(Best of 10 rounds)\n'
            '```'
        ),
        inline=False
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.MissingPermissions):
        ...
        if 'Forbidden' in str(error):
            print(f'permission error in {ctx.guild.name} - {ctx.channel.name}')
            try:
...
if __name__ == '__main__':
    ...
    if not TOKEN:
        ...
        print('set your discord bot token as TOKEN in env variables')
    else:
        print('starting bot...')
        bot.run(TOKEN)
